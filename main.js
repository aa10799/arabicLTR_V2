...
const jsPsych = initJsPsych({
    ...
    on_finish: function() {
        ...
        jsPsych.data.get().localSave('csv', 'experiment_trial1.csv');
    }
});
    ...
    timeline: [
       
        {
            ...
            stimulus:
                ...
                "<p>" +
                "This experiment should take roughly 1 hour to complete." +
                "</p>" +
                "<p>" +
...
var natural_timeline = {
    timeline: [
        {
            type: jsPsychHtmlKeyboardResponse,
            stimulus: "<div style='font-size: 32px;'> .يعمل الإنسان على استغلال موارد الطبيعة لبناء تقدمه وحضارته، إلا أن استغلاله المفرط لهذه الموارد يتم بطرق خاطئة الأمر الذي أدى إلى اختلال التوازن البيئي، وأضر البيئة بشكل عام، فأصبحت ضعيفة هشة لا تستطيع الوفاء بمتطلباته. وقد دأبت دول كثيرة تعتمد على الزراعة كمصدر للدخل إلى التركيز على زراعة الأرض أكثر من مرة في السنة الواحدة، مما أدى إلى إجهاد تربتها، إضافة إلى إزالة أجزاء كبيرة من الغابات التي تعتبر مأوى الحياة البرية فأضر ذلك بها وقلل من أعدادها بدرجة كبيرة</div><p style='font-style: italic;'>Press enter when you're done reading</p>",
            choices: ['Enter'],
            response_ends_trial: true
        },
        {
...
const completionCodeTrial = {
    type: jsPsychHtmlButtonResponse,
    stimulus: function () {
      return `<p>Your completion code is: <b>${randomCode}</b>.</p> <p>Please send an email to <b>aa10799@nyu.edu</b> with your completion code to receive your payment.</p>`;
    },
    choices: ['Done'],
    response_ends_trial: true,
  };
